...
int main()
{
	...
	outtextxy(500, 300, "HAYYP EVERY DAY");
	//welcome();
	...
	while (!kbhit())
	{
		...
		{
			if (count >= 1800 && count < 2350)
			{
				settextcolor(YELLOW);
				settextstyle(30, 0, "黑体");
				outtextxy(400, 300, "二弟给我看看实验（狗头）");
				count++;
			}
			else
				return 0;
...
void Chose(DWORD& t1)
{
	...
	if (t2 - t1 > 100)
	{
		...
		if (n < 13 && Jet[n].shoot == false && Fire[n].show == false)
		{
			...
			sprintf(c1, "open 冲天.wav alias s%d", n);
	...
	for (int i = 0; i < 13; i++)
	{
		...
		if (Jet[i].t2 - Jet[i].t1 > Jet[i].dt && Jet[i].shoot == true)
		{
			...
			if (Jet[i].y <= Jet[i].hy)
			{
				// 播放爆炸声
				char c1[50], c2[30], c3[30];
				sprintf(c1, "open y1868.wav alias n%d", i);
...
void Style(DWORD& st1)
{
	...
	if (st2 - st1 > 20000)		// 一首歌的时间
	{
		...
		for (int i = 0; i < NUM; i++)
		{
			...
			/**** 播放发射声音 ***/
			char c1[50], c2[30], c3[30];
			sprintf(c1, "open 冲天.wav alias s%d", i);
